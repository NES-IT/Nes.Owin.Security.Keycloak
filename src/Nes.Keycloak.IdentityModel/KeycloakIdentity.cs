...
using Nes.Keycloak.IdentityModel.Extensions;
using Nes.Keycloak.IdentityModel.Models.Configuration;
using Nes.Keycloak.IdentityModel.Models.Messages;
using Nes.Keycloak.IdentityModel.Models.Responses;
using Nes.Keycloak.IdentityModel.Utilities;
using Nes.Keycloak.IdentityModel.Utilities.ClaimMapping;
using Newtonsoft.Json.Linq;
using System.IdentityModel.Tokens.Jwt;

namespace Nes.Keycloak.IdentityModel
{
	public class KeycloakIdentity : ClaimsIdentity, IIdentity
	{
		...
		protected IEnumerable<Claim> GenerateJwtClaims(JwtSecurityToken accessToken, JwtSecurityToken idToken, JwtSecurityToken refreshToken)
		{
		    ...
		    if (_idToken != null)
		    {
		        var idTokenClaims = ProcessOidcToken(idToken.GetPayloadJObject(), ClaimMappings.IdTokenMappings, jsonId);
		    ...
		    if (_accessToken != null)
		    {
                var accessTokenClaims = ProcessOidcToken(accessToken.GetPayloadJObject(), ClaimMappings.AccessTokenMappings, jsonId);
		    ...
		    if (_refreshToken != null)
		    {
                var refreshTokenClaims = ProcessOidcToken(refreshToken.GetPayloadJObject(), ClaimMappings.RefreshTokenMappings, jsonId);
		...
		protected async Task CopyFromJwt(string accessToken, string refreshToken = null, string idToken = null)
		{
			...
			var accessSecurityToken = tokenHandler.ValidateToken(accessToken, _parameters, uriManager);
